...
import type { LecturaSensor, Parcela } from '../types'
...
export function ParcelaDetalle() {
  ...
  const cargarDatosParcela = async (fInicio = '', fFin = '') => {
    ...
    setError('')
    try {
      const resParcela = await axios.get<Parcela>(`/api/parcelas/${uuid}/`, {
      ...
      const resLecturas = await axios.get<LecturaSensor[]>(`/api/parcelas/${uuid}/historico/`, {
        params,
        withCredentials: true,
      })
      ...
      console.error(err)
      const msg = 'No se pudo cargar la información de la parcela.'
      setError(msg)
      toast.error(msg)
    } finally {
      setCargando(false)
      ...
      <div className="space-y-6">
        <div className="rounded-eco-lg border border-dashed border-red-200 bg-white px-6 py-16 text-center shadow-soft">
          ...
          <button
            ...
            className="mt-6 inline-flex items-center gap-2 rounded-xl bg-eco-green-primary px-5 py-3 font-bold text-white shadow-md shadow-eco-green-primary/10 transition hover:bg-eco-green-dark cursor-pointer"
          >
            ← Volver a Parcelas
          </button>
        </div>
  ...
  // Exportar CSV de lecturas
  const descargarCSV = () => {
    const headers = ['Fecha', 'Temperatura (°C)', 'Humedad (%)', 'pH']
    const rows = lecturas.map((l) => [l.fecha_registro, l.temperatura, l.humedad, l.ph])
  ...
  // Exportar JSON de lecturas
  const descargarJSON = () => {
    const jsonContent = JSON.stringify(lecturas, null, 2)
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Tarjeta de Cabecera de la Parcela */}
      <div className="rounded-eco-lg border border-slate-100 bg-white p-6 shadow-soft">
        ...
        <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <div className="rounded-eco-sm bg-eco-green-light/40 p-3">
            ...
            <p className="mt-1 text-sm font-bold text-slate-700">📊 {lecturas.length} lecturas</p>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="rounded-xl bg-eco-green-primary px-5 py-2 font-bold text-white shadow-md shadow-eco-green-primary/10 transition hover:bg-eco-green-dark text-sm cursor-pointer"
            >
              Filtrar
            </button>
            {filtroActivo && (
